...
var NextURL = '/js/calendar_next.gif';
var PrevURL = '/js/calendar_prev.gif';
var NextYearURL = '/js/calendar_nextYear.gif';
var PrevYearURL = '/js/calendar_prevYear.gif';
...
function dateObject() {
   ...
   this.fullName = this.yearValue + ' ' + this.monthName;
...
function storedMonthObject(DateFormat, DateYear, DateMonth, DateDay) {
   ...
   else if (/YYYY-MM-DD/.test(DateFormat)) this.formatted = this.yearPad +"-"+ this.monthPad +"-"+ this.dayPad;
   else {
...
// Object for the previous/next year buttons
function neighborYearObject(ParentObject, IDText, DateYear, DateMonth) {
   dateObject.call(this, DateYear, DateMonth, 1);
   this.buttonID = ParentObject.hiddenFieldName + '_' + IDText + '_ID';
   this.hover = new Function('C','O','NeighborHover(C,O,this)');
   this.getButton = new Function('return document.getElementById(this.buttonID)');
   this.go = new Function(ParentObject.objName + '.getCalendar().style.zIndex=++ZCounter;' + ParentObject.objName + '.setDisplayed(this.yearValue,this.monthIndex);');
   if (ParentObject.formNumber >= 0) this.getButton().title = this.yearValue;
}
...
function SetDisplayedMonth(DispYear, DispMonth) {
   ...
   this.previousYear = new neighborYearObject(this, 'PreviousYear', parseInt(DispYear,10)-1, DispMonth);
   this.nextYear = new neighborYearObject(this, 'NextYear', parseInt(DispYear,10)+1, DispMonth);
   ...
   if (DefaultDate == 'undefined') SetGoodDate(this, false);
      ...
      if (this.format == 'YYYYMMDD') {
         (/^\d{8}$/.test(DefaultDate)) ? this.setPicked(DefaultDate.substr(0,4), parseInt(DefaultDate.substr(4,2),10)-1, DefaultDate.substr(6,2)) : SetGoodDate(this, true);
			...
			if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(DefaultDate)) {
				var arr = DefaultDate.split('-');
				this.setPicked(arr[0], parseInt(arr[1],10)-1, arr[2]);
			}else{
         ...
         if (/\//.test(this.format)) {
            if (/^(\d{1,2})\/(\d{1,2})\/(\d{2,4})$/.test(DefaultDate)) {
         	...
         	if (/MON/.test(this.format)){
	        	...
	        	// �й�������ʾ
	            if (/^(\d{2,4})-(\d{1,2})-(\d{1,2})$/.test(DefaultDate)) {
	                  var MonPart = RegExp.$2;
	                  var DayPart = RegExp.$3;
	                  var YearPart = RegExp.$1;
	               this.setPicked(GetGoodYear(YearPart), parseInt(MonPart,10)-1, DayPart);
	            }
	            else SetGoodDate(this, true);
	     	}
         }
...
function DateInput(DateName, DateFormat, DefaultDate) {
   if (DateName == undefined) document.writeln('<span style="color:red;font-size:' + FontSize + 'px;font-family:' + FontFamily + ';">ERROR: Missing required parameter in call to \'DateInput\': [name of hidden date field].</span>');
      ...
      else if ((/^YYYYMMDD$/i.test(DateFormat)) || (/^YYYY-MM-DD$/i.test(DateFormat)) || (/^((MM?)|(DD?))\/((MM?)|(DD?))\/Y{2,4}$/i.test(DateFormat)) || (/^((DD?)|((MON)|(MMM)))-((DD?)|((MON)|(MMM)))-Y{2,4}$/i.test(DateFormat))) DateFormat = DateFormat.toUpperCase();
      else {
      ...
      with (document) {
         //writeln('<input type="text" name="' + DateName + '" value="' + InitialDate + '" onclick="javascript:' + DateName + '_Object.show()">');
         // Find this form number
         for (var f=0;f<forms.length;f++) {
            for (var e=0;e<forms[f].elements.length;e++) {
               if (typeof forms[f].elements[e].type == 'string') {
                  if ((forms[f].elements[e].type == 'text') && (forms[f].elements[e].name == DateName)) {
                  	forms[f].elements[e].value=InitialDate;
                  	forms[f].elements[e].readOnly=true;
...
//		writeln('<table cellpadding="0" cellspacing="2"><tr>' + String.fromCharCode(13) + '<td valign="middle">');
		...
		writeln('<td id="' + DateName + '_PreviousYear_ID" style="cursor:default" align="center" class="calendarDateInput" style="height:' + CellHeight + '" onClick="' + DateName + '_Object.previousYear.go()" onMouseDown="VirtualButton(this,true)" onMouseUp="VirtualButton(this,false)" onMouseOver="return ' + DateName + '_Object.previousYear.hover(this,true)" onMouseOut="return ' + DateName + '_Object.previousYear.hover(this,false)" title="' + eval(DateName + '_Object.previousYear.yearValue') + '"><img src="' + PrevYearURL + '" width="9" height="9"></td>');
		...
		writeln('<td id="' + DateName + '_Next_ID" style="cursor:default" align="center" class="calendarDateInput" style="height:' + CellHeight + '" onClick="' + DateName + '_Object.next.go()" onMouseDown="VirtualButton(this,true)" onMouseUp="VirtualButton(this,false)" onMouseOver="return ' + DateName + '_Object.next.hover(this,true)" onMouseOut="return ' + DateName + '_Object.next.hover(this,false)" title="' + eval(DateName + '_Object.next.monthName') + '"><img src="' + NextURL + '" width="5" height="9"></td>');
		writeln('<td id="' + DateName + '_NextYear_ID" style="cursor:default" align="center" class="calendarDateInput" style="height:' + CellHeight + '" onClick="' + DateName + '_Object.nextYear.go()" onMouseDown="VirtualButton(this,true)" onMouseUp="VirtualButton(this,false)" onMouseOver="return ' + DateName + '_Object.nextYear.hover(this,true)" onMouseOut="return ' + DateName + '_Object.nextYear.hover(this,false)" title="' + eval(DateName + '_Object.next.yearValue') + '"><img src="' + NextYearURL + '" width="9" height="9"></td></tr>' + String.fromCharCode(13) + '<tr>');
